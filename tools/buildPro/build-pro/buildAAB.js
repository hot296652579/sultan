    ...
    fs.readFile(ConstDefine, "utf8", function (error, content) {
        if (error) {
            ...
            fs.writeFile(ConstDefine, content, function (e) {
                ...
                console.log('开始生成aab！！！',channelId);
                // 打包命令
                exec(`cd ${androidProPath} && gradlew bundleRelease`);
                ...
                fs.readFile(grable, "utf8", function (error, content) {
                    if (error) {
                        ...
                        let apkName=`TopEntertainment-release${v}_${channelId}.aab`
                        cp(`-rf`, `${androidProPath}/app/build/outputs/bundle/release/btgame.aab`,`../../../packVersion/output/bundle/${apkName}`);
                        console.log('生成成功aab！！！',apkName);
...
_mkdirSync('../../../packVersion/output/bundle');
