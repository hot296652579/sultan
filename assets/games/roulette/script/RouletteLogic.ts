...
import RouletteView from "./view/RouletteView";

class RouletteLogic extends Logic {
    ...
    private onEnterGame(msg) {
        if (msg.bundle == this.bundle) {
            ...
            //游戏数据初始化
            Manager.gameData = Roulette.gameData;
    ...
    protected onLoadResourceComplete(err: ResourceLoaderError) {
        ...
        Manager.uiManager.open({ type: RouletteView, bundle: this.bundle, args: [this.data] }).then(() => {
            //加载完成，恢复网络
            LobbyService.instance.resumeMessageQueue();
        }).catch((e) => {
            LobbyService.instance.resumeMessageQueue();
            cc.error(`打开y游戏界面：${RouletteView.name}时报错了:`, e)
        })

    }

    protected getLoadResources(): ResourceData[] {
        return [{ preloadView: RouletteView, bundle: this.bundle }];
    }

    protected onLanguageChange() {
        super.onLanguageChange();
        Roulette.gameData.onLanguageChange();
    }
}

Manager.logicManager.push(RouletteLogic);