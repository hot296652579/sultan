...
export default class RouletteMyBetRecordView extends UIView implements IController<LobbyService> {
    ...
    // 当天历史数据列表
    private _gameRecordDataList: MST.IRouletteBetDetail[] = null;
    ...
    onLanguageChange() {
        this.titleItemPage.languagePageName(Manager.makeLanguage("labMyBetRecord", true));
        this.labResult.language = Manager.makeLanguage("labResult", true);
        this.labBetGold.language = Manager.makeLanguage("labBetGold", true);
        this.labWinChips.language = Manager.makeLanguage("labWinChips", true);
    }
     ...
     */
    private C2R_GetGameRecords_Req(index?: number): void {
        if (TypeUtils.isNull(index)) {
            index = DEFAULT_FIRST_ID;
        }

        let data: MST.C2R_GetBetInfo_Req = {
            RpcId: Manager.netManager.getNewSeqId(),
            index,
            limit: PAGE_COUNT,
        }
        let req = MST.C2R_GetBetInfo_Req.create(data);
        let buffer = MST.C2R_GetBetInfo_Req.encode(req).finish();
        this.service.sendMsg(MST.C2R_GetBetInfo_Req, MST.OuterOpcode_Roulette.C2R_GetBetInfo_Req, buffer);
    }

    private onEvent_onR2C_GetBetInfo_Res(data: MST.IR2C_GetBetInfo_Res): void {
        if (data.Error !== 0) {
