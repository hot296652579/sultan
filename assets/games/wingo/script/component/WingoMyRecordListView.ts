...
@ccclass
export default class WingoMyRecordListView extends cc.Component {
    ...
    // 原始高度
    private _originSize: cc.Size = null;
    ...
    public set<T>(itemsData: T | T[]): void {
        ...
        if (TypeUtils.isArray(itemsData)) {
            if ((itemsData as Array<T>).length <= 0) {
                this.remove(0);
                return;
            } else {
                itemsData = (itemsData as Array<T>).slice(-WingoDefine.HISTORY_COUNT);
            }
        }
    ...
    private updateListItem(): void {
        ...
        let size: cc.Size = this.getInnerEstimateSize();
     ...
     */
    public getInnerEstimateSize(): cc.Size {
